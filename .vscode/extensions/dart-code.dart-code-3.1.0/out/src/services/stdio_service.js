...
class StdIOService {
    ...
    createProcess(workingDirectory, binPath, args, envOverrides) {
        ...
        this.process = processes_1.safeSpawn(workingDirectory, binPath, args, envOverrides);
    ...
    handleResponse(evt) {
        ...
        if (handler === "CANCELLED") {
            log_1.logInfo(`Ignoring response to ${evt.id} because it was cancelled:\n\n${JSON.stringify(evt, undefined, 4)}`);
            return;
        }
        else if (!handler) {
            log_1.logError(`Unable to handle response with ID ${evt.id} because its handler is not available`);
            return;
        }
