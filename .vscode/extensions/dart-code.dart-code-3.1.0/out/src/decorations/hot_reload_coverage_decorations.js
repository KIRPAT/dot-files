...
class HotReloadCoverageDecorations {
    ...
    onDidChangeTextDocument(e) {
        if (!this.isDebugging)
            return;
        const editor = vs.window.visibleTextEditors.find((editor) => utils_2.fsPath(editor.document.uri) === utils_2.fsPath(e.document.uri));
        if (!editor)
            return;
        let fileState = this.fileState[utils_2.fsPath(e.document.uri)];
        if (!fileState) {
            fileState = this.fileState[utils_2.fsPath(e.document.uri)] = { modified: [], notRun: [] };
        }
        ...
        fileState.modified = utils_1.uniq(fileState.modified);
    ...
    redrawDecorations(editors) {
        ...
        for (const editor of editors) {
            const fileState = this.fileState[utils_2.fsPath(editor.document.uri)];
    ...
    coverageFilesUpdate() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const openFilesWithChanges = vs.window
                .visibleTextEditors
                .map((e) => utils_2.fsPath(e.document.uri))
    ...
    onReceiveCoverage(coverageData) {
        for (const data of coverageData) {
            const fileState = this.fileState[utils_2.fsPath(data.scriptPath)];
            if (!fileState)
                continue;
            const editor = vs.window.visibleTextEditors.find((editor) => utils_2.fsPath(editor.document.uri) === data.scriptPath);
