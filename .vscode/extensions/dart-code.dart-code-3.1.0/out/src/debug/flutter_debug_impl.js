...
class FlutterDebugSession extends dart_debug_impl_1.DartDebugSession {
    constructor() {
        ...
        this.logCategory = utils_1.LogCategory.FlutterRun;
    ...
    spawnProcess(args) {
        ...
        if (isAttach)
            this.sendEvent(new vscode_debugadapter_1.Event("dart.launching", { message: "Waiting for Application to connect...", finished: false }));
        else
            this.sendEvent(new vscode_debugadapter_1.Event("dart.launching", { message: "Launching...", finished: false }));
        ...
        const logger = (message, severity) => this.sendEvent(new vscode_debugadapter_1.Event("dart.log", new utils_1.LogMessage(message, severity, this.logCategory)));
        ...
        this.flutter.registerForAppStarted((n) => {
            ...
            this.connectToObservatoryIfReady();
            this.sendEvent(new vscode_debugadapter_1.Event("dart.launched"));
        });
        this.flutter.registerForAppStop((n) => { this.currentRunningAppId = undefined; this.flutter.dispose(); });
        this.flutter.registerForAppProgress((e) => this.sendEvent(new vscode_debugadapter_1.Event("dart.progress", { message: e.message, finished: e.finished, progressID: e.progressId || e.id })));
        // TODO: Should this use logToUser?
        this.flutter.registerForError((err) => this.sendEvent(new vscode_debugadapter_1.OutputEvent(`${err}\n`, "stderr")));
    ...
    spawnRunDaemon(isAttach, args, logger) {
        ...
        if (isAttach) {
            ...
            if (flutterAttach.observatoryUri) {
                if (args.flutterAttachSupportsUris) {
                    ...
                    const observatoryPort = debug_1.extractObservatoryPort(flutterAttach.observatoryUri);
                    if (observatoryPort) {
                        appArgs.push("--debug-port");
                        appArgs.push(observatoryPort.toString());
                    }
                    else {
                        log_1.logWarn(`Observatory port was not found: ${flutterAttach.observatoryUri}`);
                    }
                }
            }
        }
        ...
        return new flutter_run_1.FlutterRun(isAttach ? flutter_run_base_1.RunMode.Attach : flutter_run_base_1.RunMode.Run, args.flutterPath, args.cwd, appArgs, args.env, args.flutterRunLogFile, logger, this.maxLogLineLength);
    }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                if (this.currentRunningAppId && this.appHasStarted && !this.processExited) {
                    const quitMethod = this.flutter.mode === flutter_run_base_1.RunMode.Run
    ...
    restartRequest(response, args) {
        ...
        this.performReload(true, constants_1.restartReasonManual);
            ...
            try {
                switch (request) {
                    ...
                    case "checkPlatformOverride":
                        if (this.currentRunningAppId) {
                            const result = yield this.flutter.callServiceExtension(this.currentRunningAppId, "ext.flutter.platformOverride", undefined);
                    ...
                    case "checkIsWidgetCreationTracked":
                        if (this.currentRunningAppId) {
                            const result = yield this.flutter.callServiceExtension(this.currentRunningAppId, "ext.flutter.inspector.isWidgetCreationTracked", undefined);
                ...
                this.sendEvent(new vscode_debugadapter_1.OutputEvent(`${e}\n`, "stderr"));
            }
        });
    }
