...
class TestResultsProvider {
    constructor() {
        ...
        this.disposables.push(vs.commands.registerCommand("dart.startDebuggingTest", (treeNode) => {
            vs.debug.startDebugging(vs.workspace.getWorkspaceFolder(treeNode.resourceUri), test_1.getLaunchConfig(false, utils_2.fsPath(treeNode.resourceUri), treeNode instanceof TestTreeItem ? treeNode.test.name : treeNode instanceof GroupTreeItem ? treeNode.group.name : undefined, treeNode instanceof GroupTreeItem));
        }));
        this.disposables.push(vs.commands.registerCommand("dart.startWithoutDebuggingTest", (treeNode) => {
            vs.debug.startDebugging(vs.workspace.getWorkspaceFolder(treeNode.resourceUri), test_1.getLaunchConfig(true, utils_2.fsPath(treeNode.resourceUri), treeNode instanceof TestTreeItem ? treeNode.test.name : treeNode instanceof GroupTreeItem ? treeNode.group.name : undefined, treeNode instanceof GroupTreeItem));
        }));
    ...
    static flagSuiteStart(suitePath, isRunningWholeSuite) {
        ...
        if (isRunningWholeSuite && suitePath && path.isAbsolute(suitePath)) {
            const suite = suites[utils_2.fsPath(suitePath)];
            if (suite) {
        ...
        Object.keys(suites).forEach((p) => {
            const suite = suites[utils_2.fsPath(p)];
...
class SuiteData {
    ...
    reuseMatchingTest(currentSuiteRunNumber, test, handleOldParent) {
        ...
        // Reuse the one nearest to the source position.
        const sortedMatches = array_1.sortBy(matches, (t) => Math.abs(t.test.line - test.line));
        const match = sortedMatches.length ? sortedMatches[0] : undefined;
        if (match) {
            handleOldParent(match.parent);
            match.suiteRunNumber = this.currentRunNumber;
