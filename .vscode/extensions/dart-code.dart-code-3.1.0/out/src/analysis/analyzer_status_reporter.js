...
class AnalyzerStatusReporter {
    constructor(analyzer, workspaceContext, analytics) {
    ...
    shouldReportErrors() {
        ...
        if (this.workspaceContext.hasAnyFlutterProjects && sdks.flutter)
            return !utils_2.isStableSdk(utils_2.getSdkVersion(sdks.flutter));
        else
            return !utils_2.isStableSdk(utils_2.getSdkVersion(sdks.dart));
    }
    reportError(error, method) {
        const sdkVersion = utils_2.getSdkVersion(this.workspaceContext.sdks.dart);
        const flutterSdkVersion = utils_2.getSdkVersion(this.workspaceContext.sdks.flutter);
