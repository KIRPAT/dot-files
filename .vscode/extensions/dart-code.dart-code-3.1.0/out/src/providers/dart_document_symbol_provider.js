...
class DartDocumentSymbolProvider {
    ...
    provideDocumentSymbols(document, token) {
        return __awaiter(this, void 0, void 0, function* () {
            const outline = yield promises_1.waitFor(() => open_file_tracker_1.OpenFileTracker.getOutlineFor(document.uri), 500, 60000, token);
    ...
    convertResult(document, outline) {
        const symbol = new vscode_1.DocumentSymbol(outline.element.name || "<unnamed>", outline.element.parameters, analyzer_1.getSymbolKindForElementKind(outline.element.kind), this.getCodeOffset(document, outline), utils_1.toRange(document, outline.element.location.offset, outline.element.location.length));
